...
public class OperacionMinimo implements Sumarizador {

    // Calcula el valor numérico mínimo de una lista de celdas.
    // Solo considera celdas que contienen valores de tipo Number y no son NA.

    @Override
    public Celda<?> sumarizar(List<Celda<?>> celdas) {
        // Inicializa 'minimo' a null. Se usa Double (clase wrapper) para almacenar null.
        Double minimo = null;
        ...
        for (Celda<?> celda : celdas) {
            ...
            if (celda.getValor() instanceof Number) {
                ...
                System.out.println("Advertencia: Celda con valor no numérico ignorada en " +
                        "la búsqueda del mínimo: " + celda.getValor());
            }
        }
        ...
        // con el valor mínimo calculado (como Double) y el tipo NUMERICO.
        return new Celda(minimo, TipoDato.NUMERICO);
    }
}